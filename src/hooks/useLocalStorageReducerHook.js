...
function useLocalStorageReducer(key, defaultValue, reducer) {// key, defaultValue and reducer are this function's parameters. 
    //Make piece of state 
    const [state, dispatch] = useReducer(reducer, defaultValue, ()=>{ //reducer and defaultValue are generic placeholder names, useReducer is also taking a function with the try and catch as it's third parameter. 
        let value;
        try {
    ...
    return[state, dispatch]; //Return a value and a function (dispatch) to change it. 
}
